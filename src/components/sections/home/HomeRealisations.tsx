...
export default function Realisations() {
  return (
    <section id="realisations" className="relative overflow-hidden bg-white py-32">
      ...
      <div className="from-navy-950/20 pointer-events-none absolute inset-x-0 top-0 z-10 h-20 bg-gradient-to-b to-transparent" />
      ...
      <Container>
        <div className="flex flex-col items-start justify-between gap-8 md:flex-row md:items-end">
          <SectionHeader
            ...
            title={
              <>
                Des infrastructures <span className="text-gradient-signal">livrées</span>
                <br />
          ...
          <Link
            href="/realisations"
            className="tap-target group text-body text-navy-500 hover:text-navy-900 inline-flex shrink-0 items-center gap-2 font-semibold transition-colors"
          >
        ...
        <div className="bento-grid mt-14 grid auto-rows-[260px] gap-3 sm:grid-cols-3">
          {items.map((it, i) => (
            <motion.figure
              ...
              className={[
                "bento-card group bg-navy-950 relative cursor-pointer overflow-hidden rounded-2xl",
                it.colSpan,
                it.rowSpan,
              ]
                .filter(Boolean)
            ...
            >
              ...
              {/* Voile dégradé */}
              <div className="from-navy-950 via-navy-950/20 absolute inset-0 bg-gradient-to-t to-transparent" />

              {/* Tag */}
              <div className="absolute inset-x-0 top-0 flex items-center justify-between p-4">
                <span className="inline-flex items-center rounded-full bg-white/[0.11] px-3 py-1 text-[10px] font-bold tracking-[0.18em] text-white uppercase backdrop-blur-sm">
                  {it.tag}
                </span>
              </div>
              ...
              <figcaption className="absolute inset-x-0 bottom-0 p-5">
                <div className="text-lead leading-tight font-semibold text-white">{it.title}</div>
