    ...
    <footer className="relative overflow-hidden bg-[#020912] text-white">
      ...
      <div className="relative border-b border-white/[0.06]">
        ...
        <Container className="relative flex flex-col items-start justify-between gap-6 py-10 md:flex-row md:items-center">
          <div>
            <p className="mb-2 text-eyebrow font-bold uppercase tracking-[0.28em] text-signal-400/80">
      ...
      <Container className="relative">
        ...
        <div className="grid grid-cols-1 gap-12 py-16 sm:grid-cols-2 lg:grid-cols-12">
          ...
          <div className="lg:col-span-3">
            <Logo dark />
            <p className="mt-5 max-w-xs text-base md:text-body leading-[1.85] text-white/55">
          ...
          {/* Contact */}
          <div className="lg:col-span-3">
            <h3 className="mb-5 text-eyebrow font-bold uppercase tracking-[0.26em] text-white/55">
              Contact
            </h3>
            <div className="space-y-3.5">
              <div className="flex items-start gap-3 text-body leading-snug text-white/55">
              ...
              <Link href={`tel:${contact.phone.switchboardE164}`} className="tap-target flex items-center gap-3 text-body text-white/55 transition-colors hover:text-white">
                <PhoneCall className="size-3.5 shrink-0 text-signal-500/60" />
                {contact.phone.switchboard}
              </Link>
              <Link href={`tel:${contact.phone.directionE164}`} className="tap-target flex items-center gap-3 text-body text-white/55 transition-colors hover:text-white">
                <Smartphone className="size-3.5 shrink-0 text-signal-500/60" />
                {contact.phone.direction}
              </Link>
              <Link href={mailtoHref} className="tap-target flex items-center gap-3 text-body text-white/55 transition-colors hover:text-white">
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="space-y-3">
              {company.map((l) => (
                <li key={l.label}>
                  <Link href={l.href} className="tap-target group inline-flex items-center gap-1.5 text-body text-white/55 transition-colors hover:text-white">
                    {l.label}
                  </Link>
                </li>
              ))}
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="space-y-3">
              {legal.map((l) => (
                <li key={l.label}>
                  <Link href={l.href} className="tap-target text-body text-white/55 transition-colors hover:text-white">
                    {l.label}
                  </Link>
                </li>
              ))}
